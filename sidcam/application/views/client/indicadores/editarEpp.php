...
<?php echo form_open_multipart('client/indicadores/editarEpp', array('class'=>'cmxform', 'id'=>'updateform')); ?>
...
<div class="container">
    <div class="row">
        <div class="col-xs-12">
            <div class="row">
                ...
                <div class="col-xs-10">
                    <select name="epp_id" id="capacitacion_id" class="form-control" style="width: auto">
                        <?php if($epps){
                            foreach($epps as $row){
                                $key = 0;
                                foreach($epp_workes as $accidentados){
                                    if($accidentados->epp_id == $row->epp_id && $epp_id != $accidentados->epp_id){
                                        $key = 1;
                                    }
                                }
                                if($key == 0){
                                    if($row->epp_id == $epp_id){
                                        echo "<option selected value='$row->epp_id'> $row->title </option>";
                                    }
                                    else{
                                        echo "<option value='$row->epp_id'> $row->title </option>";
                                    }
                                }
                            }
                        } ?>
    ...
    <tr>
        ...
        <td align="left">
            ...
            echo form_hidden('epp_work_id', $epp_work_id);
