...
<h1><?php echo lang('backend_update_epp_work'); ?></h1>
...
<?php echo form_open_multipart('administrator/epp_work/updateEpp_work', array('class'=>'cmxform', 'id'=>'updateform')); ?>
<table width="80%" align="center" border="0" cellspacing="0" cellpadding="6">
    ...
    <tr>
        ...
        <td>
            <select name="epp_id" id="trabajador_id">
                <?php if($eppes){
                    foreach($eppes as $row){
                        ...
                        if($key == 0){
                            if($row->epp_id == $epp_id){
                                echo "<option id='$row->epp_id' selected value='$row->epp_id'> $row->title </option>";
                            }
                            else{
